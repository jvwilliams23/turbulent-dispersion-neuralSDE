...
void keras::LayerDropout::load_weights(std::ifstream &fin) {
  fin >> m_dropout_rate;
}
...
keras::DataChunk* keras::LayerDropout::compute_output(keras::DataChunk* dc) {
      ...
      for(unsigned int i = 0; i < y.size(); ++i) {
        for(unsigned int j = 0; j < y[0].size(); ++j) {
          for(unsigned int k = 0; k < y[0][0].size(); ++k) {
            y[i][j][k] = y[i][j][k];// * (1.0 - m_dropout_rate);
          }
        }
      }
      keras::DataChunk *out = new keras::DataChunk2D();
...
keras::DataChunk* keras::LayerActivation::compute_output(keras::DataChunk* dc) {
  ...
  if (dc->get_data_dim() == 3) {
    ...
    if(m_activation_type == "relu") {
      ...
      for(unsigned int k = 0; k < y.size(); ++k) {
    y[k] = 1/max(1.0e-4,(double)(1+exp(-y[k])));
        //y[k] = 1/(1+exp(-y[k]));
      }
    ...
    } else if(m_activation_type == "softplus") {
      //printf("softplus\n");
      for(unsigned int k = 0; k < y.size(); ++k) {
        //printf("y[%i] is %f \n", k, y[k]); 
        // cutoff for when softplus returns linear function
        // prevents really high values from exp()
        if (y[k] < 33.256)
        {
            y[k] = log(  max(1.0e-4, (double)(1+exp(y[k])))  );
        }
        else
        {
            continue;
    ...
    } else if(m_activation_type == "leaky_relu") {
      for(unsigned int k = 0; k < y.size(); ++k) {
        if(y[k] < 0)
        {
            y[k] = 0.2*y[k];
...
keras::DataChunk* keras::LayerDense::compute_output(keras::DataChunk* dc) {
  ...
  //m_dropout_rate = 0.1;
  //printf("DROPOUT RATE IS %f \n", m_dropout_rate);
  
  float m_dropout_rate = 0.0; // not working with dropout for now - do not use

  bool m_verbose = false; //false;
  for (size_t j = 0; j < m_weights.size(); ++j) { // iter over input
    ...
    for (size_t i = 0; i < size8; ++i) { // iter over neurons
      y_ret[k]   += w[k]   * p * (1.0 - m_dropout_rate);          // vectorize if you can
      y_ret[k+1] += w[k+1] * p * (1.0 - m_dropout_rate);
      y_ret[k+2] += w[k+2] * p * (1.0 - m_dropout_rate);
      y_ret[k+3] += w[k+3] * p * (1.0 - m_dropout_rate);
      y_ret[k+4] += w[k+4] * p * (1.0 - m_dropout_rate);
      y_ret[k+5] += w[k+5] * p * (1.0 - m_dropout_rate);
      y_ret[k+6] += w[k+6] * p * (1.0 - m_dropout_rate);
      y_ret[k+7] += w[k+7] * p * (1.0 - m_dropout_rate);
      k += 8;
    }
  ...
  // Always reset dropout back to zero after passing through the layer after dropout layer
...
std::vector<float> keras::KerasModel::compute_output(keras::DataChunk *dc) {
  ...
  for(int l = 0; l < (int)m_layers.size(); ++l) {
    if(debugParticles) cout << "Processing layer " << m_layers[l]->get_name() << " " << l << endl;
