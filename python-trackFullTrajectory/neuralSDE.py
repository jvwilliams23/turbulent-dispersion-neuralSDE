...
parser.add_argument(
  "-t",
  "--test_dict_key",
  default="Re33FinerWriteInterval",
  type=str,
  help=(
...
def read_filtered_data_csv(
  fname, max_num_p=10000000, cutoff_time=100000
):
  """
...
def format_data(
  ...
  print("timescale norm with scaler [mean, max]")
  print(x_data[:, :, timescale_inds].mean(), x_data[:, :, timescale_inds].max())
  ...
  print("mesh size norm with scaler [mean, max]")
  print(x_data[:, :, mesh_size_ind].mean(), x_data[:, :, mesh_size_ind].max())
...
if args.retrain:
  ...
  epsilon_list = [None] * train_list_len
  i = 0
  ...
  for j, (t, x, y) in enumerate(zip(time_interval, x_interval, y_interval)):
    # time_interval_list.append(np.swapaxes(t, 0, 1))
    # x_interval_list.append(np.swapaxes(x, 0, 1))
    # y_interval_list.append(np.swapaxes(y, 0, 1))
    # viscosity_arr.append(np.swapaxes(viscosity_list[j], 0, 1))
    time_interval_list.append(t)
    x_interval_list.append(x)
    y_interval_list.append(y)
    viscosity_arr.append(viscosity_list[j])
    epsilon_arr.append(epsilon_list[j])
    print(t.shape)
    print(time_interval_list[-1].shape)
    print()

  step_sizes_train = copy(time_interval)
  time_interval = np.stack(time_interval_list, axis=1)
  viscosity_arr = np.stack(viscosity_arr, axis=1)
  epsilon_arr = np.stack(epsilon_arr, axis=1)
  x_interval = np.stack(x_interval_list, axis=1)
  y_interval = np.stack(y_interval_list, axis=1)
  step_sizes_train = np.stack(step_sizes_train, axis=1)
  ...
  t0 = time()
  number_of_timesteps = step_sizes_train.shape[-1]
else:
...
log_dir_b = 'logs/batch_level/' + datetime.now().strftime("%Y%m%d-%H%M%S") + '/train'
train_writer = tf.summary.create_file_writer(log_dir_b)
...
model = neuralSDEModel(
  ...
  number_of_cases=train_list_len,
)
model.compile(
  optimizer=OPTIMIZER,
...
# checkpoint_path = WEIGHTS_PATH+"/cp-{epoch:04d}.ckpt"
checkpoint_dir = os.path.dirname(checkpoint_path)
...
if args.retrain:
  ...
  if RUNNING_LOCAL:
    notification.notify(
      title="NN finished training",
      message=f"NN with weights in {WEIGHTS_DIR} is over",
    )
...
(times_arr_base, step_sizes, x_data, y_data) = read_filtered_data_csv(
  f"{data_dir_base}/{test_dict['data_file'].format(filter_width=args.test_filt[:-1])}",
...
for vel_test, vel_network, plot_str in zip(
  ...
  mean_tke_truth_dimensional = tke_test.mean(axis=0)
  ...
  mean_tke_nn_dim = tke_network.mean(axis=0)
  ...
  tke_loss = abs(mean_tke_truth_dimensional - mean_tke_nn_dim) / mean_tke_truth_dimensional[0]
  print(f"loss is {tke_loss[20:].sum()} ")
  print(f"all loss {tke_loss}")
...
if args.plots:
  ...
  try:
    ...
    print("no entry found for model.encoder_diffusion.weights")
    pass
